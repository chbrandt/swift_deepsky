...
*-
	  ...
	  !ticket#344 fixed
      if (real(arcsec) .eq. 60.0) then
          arcsec = 0.0
          arcmin = arcmin + 1
      endif
      if (arcmin .eq. 60) then
          arcmin = 0
          !i gradi vanno sempre incrementati (il numero viene trattato in 
		  !assoluto e alla fine gli si antepone il simbolo -)
		  !if (a .eq. ' ') then
          !    degree = degree - 1
          !else
              degree = degree + 1
